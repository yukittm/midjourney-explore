...
**Also pulled forward (2026-06-27): feed-grid preview — `feed_preview.py` + `igpub/feed.py`.** Generates a self-contained `automation/feed-preview.html` (3-col IG-style grid, 4:5 tiles) projecting the profile feed: approved → scheduled (publish_at desc) → published (published_at desc), newest top-left. Read-only; `deleted_from_ig` records are excluded (a new first-class `Post.deleted_from_ig` field replaces the old manual marker). For judging color flow / anti-sameness across the grid before posting. (This is the chosen "UI" — static-generated, no server, per the no-runtime-infra stance.)
