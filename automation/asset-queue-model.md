...
**MINIMAL — ships the first posts (images + carousels; no video):** `Asset`/`Provenance` model + `Post.assets` + `STORY` enum + `schema_version` + back-compat `from_dict` (synthesize assets from old `images`; the approved→scheduled migration) · `schedule.py` (`assign_slots` pure + `plan`) · type-dispatched `validate.py` (image/carousel; hard-reject mixed AR; `duplicate_ids`) · `publish.py` (`is_due`→SCHEDULED + `id_publishable` `--id` gate + `TypeError`-safe + HEAD probe + **stale-`publishing` STOP** + container-id-saved-before-publish; `move_to_published` no-overwrite) · runners coerce naive `--now`→UTC · raise the rate self-cap · **non-expiring token** (regen) + optional 20-line `doctor` · infra fixes (config pages_base, `.gitignore` video, Pages root). CLI subset: the 3 root scripts `validate.py` + `plan.py` + `publish.py`. **Status: implemented + independently cross-checked (3 review agents → blockers fixed); 92 tests green (yaml round-trip skipped locally → CI); validated in production on 2 published posts (2026-06-25, 2026-06-27).**
**FULL — when video/Reels or scale actually arrives:** R2 host adapter + `upload_video` + `automation/video/` lane + `uploaded.json` · `reconcile.py` · full `error_class` taxonomy + retry policies · the rest of the CLI (`status/calendar/hold/pin/doctor` — `status` deferred while the queue is `ls`-shallow) · `targets[]` + per-target `results` map (cross-post) · `insights{}` write-back.

**Pulled forward (2026-06-27): `new` — `new_post.py` + `igpub/scaffold.py`.** Scaffolds a draft IMAGE post from a picked image (e.g. from `outputs/selects/`): forms `<date>_<slug>`, creates `automation/assets/<id>/01.jpg` + a draft `automation/queue/<id>.yml` (via `models.Post`→`save_post`, so the file shape can't drift). All preconditions checked before any write (kebab slug, zero-padded date, image exists, **JPEG detected by content — non-JPEG refused, never written under a `.jpg` name**, no id/asset collision); mid-write failure cleans up the partial asset dir; validates the draft; optional `--remove-select` keeps the `selects/` "picked-but-not-queued" invariant. Caption/alt stay human (the `caption` skill). `status` (the up-next view) stayed deferred — a 3-agent plan review found it premature against the empty/shallow queue + the model's own YAGNI stance.

**Also pulled forward (2026-06-27): feed-grid preview — `feed_preview.py` + `igpub/feed.py`.** Generates a self-contained `automation/feed-preview.html` (3-col IG-style grid, 4:5 tiles) projecting the profile feed: approved → scheduled (publish_at desc) → published (published_at desc), newest top-left. Read-only; `deleted_from_ig` records are excluded (a new first-class `Post.deleted_from_ig` field replaces the old manual marker). For judging color flow / anti-sameness across the grid before posting. (Static-generated, no server.) Plus **`contact_sheet.py` + `igpub/contact.py`** — a static contact-sheet of `outputs/candidates/` grouped by subject, for triage.

**Feed Studio (2026-06-28): `studio.py` + `igpub/studio_logic.py` + `studio/static/` (vendored SortableJS).** An interactive LOCAL console (127.0.0.1, stdlib `http.server`, run on demand — not publish-path infra; the no-runtime-infra stance was about the *publish* path, this is a local editing tool). Pool tab: browse candidates (group by subject/aspect/flat, hide-staged toggle), "→ selects" converts PNG→JPEG into `outputs/selects/` (Pillow, lazy; no crop). Feed tab: IG-style grid of live[locked] + selects[drag-reorder via SortableJS `animation`, order in `outputs/selects/.order.json`, gitignored], "× → pool" (un-select), "→ queue" scaffolds a draft **carrying the grid position as `priority`** (so the arrangement reaches `plan.py` — `scaffold_post` gained a `priority` arg; `build_tiles(include_drafts=True)` keeps committed drafts visible). Captions show under committed posts. Order writes are `threading.Lock`-serialized + atomic (`os.replace`). 132 tests. Caption/crop/approve/publish stay OUT of the studio (human + `caption` skill + `plan.py`/`publish.py`).
