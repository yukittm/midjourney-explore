...
automation/
  ...
  published/<YYYY>/<id>.yml   # year-sharded archive (write-back: media_id/permalink/insights)
  igpub/          # core package
...
- **`schedule.yml`**: `slots[]` (HH:MM local + weekday filter + allowed media_types), `rate{max_per_24h, min_gap_minutes}`, `horizon_days`, `blackout[]`/`skip_dates[]`. Wall-clock + IANA `timezone` (DST-correct).
