...
- A prompt template pins the base structure on the realism **kernel** + a single bold image `--sref`:
  `[shot] of [photoreal subject + action], [real terrain/phenomenon = the register], editorial photograph, [lens], [film stock] --style raw --s ~110 --sw 150–250 --ar 4:5 --no painting, illustration, 3d render, cgi`
  (set the `--sref` **image via the UI** — don't paste `--sref <URL>` into the body; **omit `--sv`** on V8.1 — it defaults to sv7)
